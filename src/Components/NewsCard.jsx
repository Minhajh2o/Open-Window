...
const NewsCard = ({ news }) => {
  const { _id, title, author, thumbnail_url, details, rating, total_view } =
    news;
  ...
  return (
    <div className="card bg-base-100 shadow-md border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="px-4 pt-3">
        <Link to={`/news/${_id}`} className="font-bold text-lg text-gray-800 hover:underline">{title}</Link>
      </div>

      {/* Thumbnail */}
      <Link to={`/news/${_id}`} className="px-4 py-3 w-full h-80">
        <img
          src={thumbnail_url}
          alt={title}
